...
source("/projects/my-public-bucket/GEDI_global_PA_v2/vl_GEDI-PA_2024/matching_func_2025.R")
...
matching_tifs <- c("d2roads","dcities","dem","slope",
                   "pop_cnt_2000","pop_den_2000","tt2cities_2000","wc_prec_1990-1999",
                   "wc_tmax_1990-1999","wc_tavg_1990-1999","wc_tmin_1990-1999",
                   "MapBiomas_brasil_coverage_2000") ##"wwf_biomes","wwf_ecoreg","lc2000",
...
matched_all <- read.csv(s3_get(paste(f.path,"WDPA_matching_results/",iso3,"_wk25_filelist.csv",sep="")))[,2]
...
if(flag=="run all"){  #determine how many PAs to run the extraction process
  ...
  runPA_id <- setdiff(matched_PAdf[,"matched_PA_id"],extracted_PAid)
...
##    matched_PAs <- matched_PAdf[matched_PAdf[,"matched_PA_id"] != extracted_PAid, "matched_PAs"]
    matched_PAs <- matched_PAdf[matched_PAdf[,"matched_PA_id"] == runPA_id, "matched_PAs"]
  } else {
    matched_PAs <- NULL
...
foreach(this_rds=matched_PAs, .combine = foreach_rbind, .packages=c('sp','magrittr', 'dplyr','tidyr')) %dopar% {
  ...
  id_pa <- basename(this_rds) %>% readr::parse_number() %>% unique()
  matched <- readRDS(s3_get(paste(f.path,"WDPA_matching_results/",iso3,"_wk",gediwk,"/",iso3,"_pa_",id_pa,"_matching_results_wk",gediwk,".RDS",sep="")))
    ### "lat", "lon", "UID", "status", "land_cover", "elevation", "slope", "mean_temp", "max_temp",
    ### "min_temp", "prec", "d2road", "d2city", "popden", "tt2city", "popcnt", "propensity_score",
  ...
  if (is.null(matched)==TRUE  | nrow(matched)==0) {
    ...
    if(table(mras$status[])[2]==0 | table(mras$status[])[1]==0 | is.null(mras)){
      ...
      iso_matched_gedi <-  iso_matched_gedi %>%
            dplyr::select("pa_id","status","land_cover","pft","region",
                          "shot_number","lat_lowestmode","lon_lowestmode",
                          "rh98","agbd","agbd_se") #"filename","rh25","rh50","rh75","wwfbiom","wwfecoreg",        
...
##    }
    ...
    landcover <- unique(iso_matched_gedi$land_cover) %>% getmode()
    ...
    saveRDS(iso_matched_gedi, file=paste("/projects/my-public-bucket/GEDI_global_PA_v2/WDPA_extract/",iso3,"_wk",gediwk,"/",iso3,"_pa_", id_pa,"_gedi_wk_",gediwk,"_conti_",continent,"_lc_",landcover,".RDS", sep=""))
