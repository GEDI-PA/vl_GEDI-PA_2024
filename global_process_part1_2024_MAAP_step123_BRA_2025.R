...
#  filename_out <- paste("output/",iso3,"_grid_wk",gediwk,".RDS", sep="")
  filename_out <- paste("/projects/my-public-bucket/GEDI_global_PA_v2/",iso3,"_grid_wk",gediwk,".RDS", sep="")
...
#      dir.create(paste(f.path,"WDPA_matching_points/",iso3,"/",sep=""))}    
  ...
  d_control <- data.frame(d_control) %>%
    dplyr::rename(
      #land_cover = lc2000,
      land_cover = MapBiomas.brasil.coverage.2000,
      slope = slope,
      elevation = dem,
      popden = pop.den.2000,
      popcnt=pop.cnt.2000,
  ...
  d_control$land_cover <- factor(d_control$land_cover, levels=sequence(10),
                                 #labels = c("l1_forest","l2_grassland",
                                 #"l3_agriculture","l4_wetlands","l5_artificial",
                                 #"l6_other land/bare","l7_water"))
                                 labels = c("l1_forest",
                                            "l2_savanna",
                                            "l3_mangrove",
                                            "l4_floodedforest",
  ...
  for(i in 1:length(allPAs)){
    ...
    testPA_spdf$y <- geom(testPA_spdf)[,"y"]
      ...
      d_pa$land_cover <- factor(d_pa$land_cover, levels=sequence(10),
                                #labels = c("l1_forest","l2_grassland","l3_agriculture",
                                #"l4_wetlands","l5_artificial","l6_other land/bare","l7_water"))
                                 labels = c("l1_forest",
                                            "l2_savanna",
                                            "l3_mangrove",
                                            "l4_floodedforest",
